...
const OrderCalculation = (props) => {
  ...
  const handleDeliveryFee = () => {
    ...
    setDeliveryFee(
      orderType === "delivery" || orderType === "schedule_order" ? price : 0
    );
    if (price === 0) {
  ...
  const handleOrderAmount = () => {
    let totalAmount = getCalculatedTotal(
      ...
      surgePrice,
      backendDeliveryCharge !== null ? backendDeliveryCharge : undefined
    );
  ...
  return (
    <>
      <CalculationGrid container item xs={12} spacing={1} mt="1rem">
        ...
        {
          Number(taxAmount?.tax_amount || 0) > 0 &&
          Number(taxAmount?.tax_included ?? 0) === 0 ? (
            <>
              <Grid item md={8} xs={8}>
                {t("VAT/TAX")}
              </Grid>
