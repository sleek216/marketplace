...
const WalletFundBonus = () => {
  ...
  const isPercentageBonus = (item) => {
    const type = `${item?.bonus_type ?? item?.type ?? ""}`.toLowerCase();
    return (
      type === "percentage" ||
      type === "percent" ||
  ...
  const getBonusDescription = (item) => {
    const minimumAmount = getAmountWithSign(item?.minimum_add_amount);
    if (isPercentageBonus(item)) {
      const percent = Number(item?.bonus_amount ?? item?.bonus ?? 0);
      return t("Add Fund to Wallet minimum and enjoy percent bonus", {
        min: minimumAmount,
        percent: percent.toFixed(1),
      });
    }
    const bonusAmount = getAmountWithSign(
      item?.bonus_amount ?? item?.bonus ?? item?.maximum_bonus_amount
    );
    return t("Add Fund to Wallet minimum and enjoy amount bonus", {
      min: minimumAmount,
      amount: bonusAmount,
    });
  };
