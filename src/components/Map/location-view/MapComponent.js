...
import { Store as StoreIcon, Plus as AddIcon, Minus as RemoveIcon } from "lucide-react";
import { getBlueGooglePinIcon } from "utils/mapMarkerIcon";
...
const MapComponent = (props) => {
  ...
  const bluePinIcon = useMemo(() => {
    ...
    return getBlueGooglePinIcon({
      google: window.google,
      fillColor: theme.palette.primary.main,
      width: 30,
      height: 40,
    });
  }, [isLoaded, theme.palette.primary.main]);
    ...
    <Stack>
      ...
      >
        <IconButton
          ...
          disabled={zoom > 21}
        >
          <AddIcon size={18} strokeWidth={2.25} />
        </IconButton>
        <IconButton
          sx={{
            background: (theme) => theme.palette.neutral[100],
          ...
          disabled={zoom < 1}
        >
          <RemoveIcon size={18} strokeWidth={2.25} />
        </IconButton>
      </Stack>
      <GoogleMap
