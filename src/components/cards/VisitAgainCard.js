...
const KmShowing = ({ distance }) => {
	...
	const normalizeDistanceKm = (rawDistance) => {
		...
		// Visit-again payload can return meters or kilometers depending on endpoint.
		return parsed > 100 ? parsed / 1000 : parsed;
	};
	const distanceKm = normalizeDistanceKm(distance);
	...
	return (
		...
		>
			...
			<Typography variant={IsSmallScreen() ? "body3" : "body1"}>
				<b>
					{distanceKm === 0 ? "0km " : `${distanceKm.toFixed(1)}km `}
				</b>{" "}
				{t("From You")}
			</Typography>
		</Stack>
			...
			sx={{
				...
				"&:hover": {
					...
					boxShadow: theme.palette.mode === "light"
						? `0px 12px 24px ${alpha(theme.palette.neutral[900], 0.1)}`
