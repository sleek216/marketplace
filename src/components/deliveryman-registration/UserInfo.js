...
const UserInfo = ({
  ...
  const deliveryTypeIconColor =
    deliveryManFormik.touched.earning && !deliveryManFormik.errors.earning
      ? theme.palette.primary.main
      : alpha(theme.palette.neutral[400], 0.7);

  const getDeliverymanTypeIcon = () => {
    if (deliveryManFormik.values.earning === "1") {
      return <PaidIcon sx={{ color: deliveryTypeIconColor, fontSize: "18px" }} />;
    }
    if (deliveryManFormik.values.earning === "0") {
      return <WorkOutlineIcon sx={{ color: deliveryTypeIconColor, fontSize: "18px" }} />;
    }
    return <PeopleIcon sx={{ color: deliveryTypeIconColor, fontSize: "18px" }} />;
  };

  const vehicleIconColor =
    deliveryManFormik.touched.vehicle_id && !deliveryManFormik.errors.vehicle_id
      ? theme.palette.primary.main
      : alpha(theme.palette.neutral[400], 0.7);

  const getVehicleIcon = () => {
    const selectedVehicle = vehicleList?.find(
      (item) => item?.id?.toString() === deliveryManFormik.values.vehicle_id
    );
    const vehicleType = selectedVehicle?.type?.toLowerCase() || "";

    if (vehicleType.includes("bike") || vehicleType.includes("bicycle")) {
      return (
        <DirectionsBikeIcon sx={{ color: vehicleIconColor, fontSize: "18px" }} />
      );
    }
    if (
      vehicleType.includes("motor") ||
      vehicleType.includes("scooter") ||
      vehicleType.includes("cycle")
    ) {
      return <TwoWheelerIcon sx={{ color: vehicleIconColor, fontSize: "18px" }} />;
    }
    if (vehicleType.includes("truck") || vehicleType.includes("van")) {
      return <LocalShippingIcon sx={{ color: vehicleIconColor, fontSize: "18px" }} />;
    }

    return <DirectionsCarIcon sx={{ color: vehicleIconColor, fontSize: "18px" }} />;
  };
            ...
            <Grid container columnSpacing={3}>
              ...
              <Grid item xs={12} sm={6} sx={{ minHeight: "5rem" }}>
                <CustomSelectWithFormik
                  ...
                  startIcon={getDeliverymanTypeIcon()}
                />
              </Grid>
