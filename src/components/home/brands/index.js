...
const Brands = ({ viewAll }) => {
	...
	// Only refetch on mount / module change — never when `data` updates
	// (that caused an infinite refetch → permanent loading state).
		...
		<>
			{data?.length > 0 ? (
				<MarketplaceSectionHeader
					title="Brands"
					subtitle="Explore the Trusted and Trendsetting Brands"
					mb={0}
					onSeeAll={() => router.push("/all-brands")}
				/>
			) : isLoading ? (
				<>
					<Skeleton variant="text" width="80px" />
