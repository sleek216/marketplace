...
const FoodDetailModal = ({
  ...
  useEffect(() => {
    const itemId = fromCard?.id || data?.id || modalData?.[0]?.id;
  ...
  }, [open, fromCard?.id, data?.id, modalData?.[0]?.id]);
    ...
    if (choiceType === "single") {
      if (checked) {
        if (selectedOptions.length > 0) {
          ...
          if (isExist) {
            const newSelectedOptions = selectedOptions.filter(
              (sOption) =>
                !(sOption.choiceIndex === choiceIndex && sOption.label === isExist.label)
            );
        ...
        const filtered = selectedOptions.filter(
          (item) => !(item.choiceIndex === choiceIndex && item.label === option.label)
        );
