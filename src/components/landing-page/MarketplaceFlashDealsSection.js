import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { useTranslation } from "react-i18next";
import { useSelector } from "react-redux";
import {
  alpha,
  Box,
  IconButton,
  Skeleton,
  Stack,
  Typography,
  useMediaQuery,
  useTheme,
} from "@mui/material";
import ChevronLeftIcon from "@mui/icons-material/ChevronLeft";
import ChevronRightIcon from "@mui/icons-material/ChevronRight";
import useMarketplaceFlashSales from "api-manage/hooks/react-query/landing/useMarketplaceFlashSales";
import LandingProductCard from "components/landing-page/LandingProductCard";
import ModuleModal from "components/cards/ModuleModal";
import FoodDetailModal from "components/food-details/foodDetail-modal/FoodDetailModal";
import CustomContainer from "components/container";
import { ModuleTypes } from "helper-functions/moduleTypes";
import {
  marketplaceStripCardSx,
  marketplaceStripGapSx,
} from "./marketplaceCardLayout";

const FlashCardSkeleton = () => {
  const theme = useTheme();
  return (
    <Box
      sx={{
        ...marketplaceStripCardSx,
        borderRadius: "2px",
        overflow: "hidden",
        border: `1px solid ${alpha(theme.palette.divider, 0.35)}`,
        bgcolor: "background.paper",
      }}
    >
      <Skeleton variant="rectangular" sx={{ pt: "85%" }} />
      <Stack spacing={0.4} sx={{ px: 1, pt: 0.6, pb: 0.85 }}>
        <Skeleton variant="text" width="40%" height={10} />
        <Skeleton variant="text" width="90%" height={13} />
        <Skeleton variant="text" width="55%" height={16} />
        <Skeleton variant="rectangular" width="100%" height={30} sx={{ borderRadius: "2px" }} />
      </Stack>
    </Box>
  );
};

/**
 * Landing Flash Deals — GET /api/v1/marketplace/flash-sales/items
 * (zoneId only, no moduleId). Renders above All Products.
 */
const MarketplaceFlashDealsSection = () => {
  const { t } = useTranslation();
  const theme = useTheme();
  const scrollRef = useRef(null);
  const { configData } = useSelector((state) => state.configData);
  const { wishLists } = useSelector((state) => state.wishList);

  const [mounted, setMounted] = useState(false);
  const [zoneKey, setZoneKey] = useState("");
  const [modalPayload, setModalPayload] = useState(null);
  const [canScrollLeft, setCanScrollLeft] = useState(false);
  const [canScrollRight, setCanScrollRight] = useState(false);
  const [isStripHovered, setIsStripHovered] = useState(false);
  const isDesktop = useMediaQuery(theme.breakpoints.up("md"));

  const syncZone = useCallback(() => {
    if (typeof window === "undefined") return;
    setZoneKey(localStorage.getItem("zoneid") || "");
  }, []);

  useEffect(() => {
    setMounted(true);
    syncZone();
  }, [syncZone]);

  useEffect(() => {
    if (!mounted) return undefined;
    const onStorage = (e) => {
      if (!e.key || e.key === "zoneid" || e.key === "location") syncZone();
    };
    window.addEventListener("storage", onStorage);
    window.addEventListener("focus", syncZone);
    const interval = window.setInterval(syncZone, 2000);
    return () => {
      window.removeEventListener("storage", onStorage);
      window.removeEventListener("focus", syncZone);
      window.clearInterval(interval);
    };
  }, [mounted, syncZone]);

  const { data, isLoading, isFetching, isError, refetch } =
    useMarketplaceFlashSales({
      enabled: mounted && Boolean(zoneKey),
      zoneKey,
      limit: 20,
    });

  const products = useMemo(() => data?.products || [], [data]);

  const updateScrollState = useCallback(() => {
    const el = scrollRef.current;
    if (!el) return;
    const maxScroll = el.scrollWidth - el.clientWidth;
    setCanScrollLeft(el.scrollLeft > 4);
    setCanScrollRight(el.scrollLeft < maxScroll - 4);
  }, []);

  useEffect(() => {
    updateScrollState();
    const el = scrollRef.current;
    if (!el) return undefined;
    el.addEventListener("scroll", updateScrollState, { passive: true });
    window.addEventListener("resize", updateScrollState);
    return () => {
      el.removeEventListener("scroll", updateScrollState);
      window.removeEventListener("resize", updateScrollState);
    };
  }, [products, isLoading, updateScrollState]);

  const scrollStrip = (direction) => {
    const el = scrollRef.current;
    if (!el) return;
    const amount = Math.max(el.clientWidth * 0.7, 220);
    el.scrollBy({ left: direction * amount, behavior: "smooth" });
  };

  const handleRequestDetail = useCallback((bundle) => {
    setModalPayload(bundle);
  }, []);

  const closeModal = useCallback(() => {
    setModalPayload(null);
  }, []);

  const getIsWishlisted = useCallback(
    (itemId) =>
      Boolean(wishLists?.item?.some((wishItem) => wishItem?.id === itemId)),
    [wishLists]
  );

  const isFoodItem = (item) =>
    item?.module?.module_type === ModuleTypes.FOOD ||
    item?.module_type === ModuleTypes.FOOD;

  // Always show section once mounted so landing layout is stable.
  if (!mounted) return null;

  const showLoading = Boolean(zoneKey) && isLoading && products.length === 0;
  const showEmpty =
    Boolean(zoneKey) && !isLoading && !isFetching && !isError && products.length === 0;
  const showError = Boolean(zoneKey) && isError && products.length === 0;
  const showNav = isDesktop && products.length > 0 && (canScrollLeft || canScrollRight);

  const navSx = (visible) => ({
    position: "absolute",
    top: "40%",
    transform: "translateY(-50%)",
    zIndex: 3,
    width: 36,
    height: 36,
    borderRadius: "2px",
    bgcolor: "background.paper",
    border: `1px solid ${alpha(theme.palette.divider, 0.35)}`,
    boxShadow: `0 2px 10px ${alpha(theme.palette.common.black, 0.1)}`,
    opacity: visible && isStripHovered ? 1 : 0,
    pointerEvents: visible && isStripHovered ? "auto" : "none",
    transition: "opacity 0.2s ease",
    "&:hover": {
      bgcolor: "primary.main",
      color: "primary.contrastText",
      borderColor: "primary.main",
    },
  });

  return (
    <>
      <Box
        component="section"
        sx={{
          bgcolor: alpha(
            theme.palette.primary.main,
            theme.palette.mode === "dark" ? 0.12 : 0.05
          ),
          pt: { xs: 2, md: 2.5 },
          pb: { xs: 2, md: 2.5 },
        }}
      >
        <CustomContainer>
          <Stack
            direction="row"
            alignItems="center"
            justifyContent="space-between"
            spacing={1.5}
            mb={1.5}
          >
            <Box minWidth={0}>
              <Typography
                component="h2"
                sx={{
                  fontWeight: 700,
                  fontSize: { xs: "16px", md: "18px" },
                  lineHeight: 1.2,
                  color: "text.primary",
                }}
              >
                {t("Flash Deals")}
              </Typography>
              <Typography
                sx={{
                  mt: 0.2,
                  fontSize: { xs: "11px", sm: "12px" },
                  color: "text.secondary",
                  display: { xs: "none", sm: "block" },
                }}
              >
                {t("Limited-time deals across the marketplace")}
              </Typography>
            </Box>
            {data?.total_size > 0 && (
              <Typography
                sx={{
                  fontSize: "12px",
                  fontWeight: 600,
                  color: "primary.main",
                  flexShrink: 0,
                }}
              >
                {data.total_size} {t("deals")}
              </Typography>
            )}
          </Stack>

          {!zoneKey ? (
            <Box
              sx={{
                py: 3,
                px: 2,
                textAlign: "center",
                borderRadius: "2px",
                bgcolor: "background.paper",
                border: `1px solid ${alpha(theme.palette.divider, 0.3)}`,
              }}
            >
              <Typography color="text.secondary" fontSize={13}>
                {t("Select location first to see flash deals")}
              </Typography>
            </Box>
          ) : showError ? (
            <Box
              sx={{
                py: 3,
                px: 2,
                textAlign: "center",
                borderRadius: "2px",
                bgcolor: "background.paper",
                border: `1px solid ${alpha(theme.palette.divider, 0.3)}`,
              }}
            >
              <Typography color="text.secondary" fontSize={13} mb={1}>
                {t("Could not load flash deals. Please try again.")}
              </Typography>
              <Typography
                component="button"
                onClick={() => refetch()}
                sx={{
                  all: "unset",
                  cursor: "pointer",
                  color: "primary.main",
                  fontWeight: 700,
                  fontSize: 13,
                }}
              >
                {t("Retry")}
              </Typography>
            </Box>
          ) : showEmpty ? (
            <Box
              sx={{
                py: 3,
                px: 2,
                textAlign: "center",
                borderRadius: "2px",
                bgcolor: "background.paper",
                border: `1px solid ${alpha(theme.palette.divider, 0.3)}`,
              }}
            >
              <Typography color="text.secondary" fontSize={13}>
                {t("No flash deals available right now")}
              </Typography>
            </Box>
          ) : (
          <Box
            onMouseEnter={() => setIsStripHovered(true)}
            onMouseLeave={() => setIsStripHovered(false)}
            sx={{ position: "relative" }}
          >
            {showNav && (
              <>
                <IconButton
                  aria-label="Previous flash deals"
                  onClick={() => scrollStrip(-1)}
                  sx={{ ...navSx(canScrollLeft), left: 0 }}
                >
                  <ChevronLeftIcon />
                </IconButton>
                <IconButton
                  aria-label="Next flash deals"
                  onClick={() => scrollStrip(1)}
                  sx={{ ...navSx(canScrollRight), right: 0 }}
                >
                  <ChevronRightIcon />
                </IconButton>
              </>
            )}

            <Box
              ref={scrollRef}
              sx={{
                display: "flex",
                gap: marketplaceStripGapSx,
                overflowX: "auto",
                overflowY: "hidden",
                scrollSnapType: "x mandatory",
                WebkitOverflowScrolling: "touch",
                scrollbarWidth: "none",
                msOverflowStyle: "none",
                "&::-webkit-scrollbar": { display: "none" },
                pb: 0.5,
              }}
            >
              {showLoading
                ? Array.from({ length: 6 }).map((_, i) => (
                    <FlashCardSkeleton key={`flash-skel-${i}`} />
                  ))
                : products.map((item) => (
                    <Box
                      key={`flash-${item?.flash_sale_item_id || item?.id}-${item?.module_id || ""}`}
                      sx={marketplaceStripCardSx}
                    >
                      <LandingProductCard
                        item={item}
                        onRequestDetail={handleRequestDetail}
                      />
                    </Box>
                  ))}
            </Box>
          </Box>
          )}
        </CustomContainer>
      </Box>

      {modalPayload && isFoodItem(modalPayload.item) && (
        <FoodDetailModal
          product={modalPayload.item}
          imageBaseUrl={modalPayload.imageBaseUrl}
          open
          handleModalClose={closeModal}
          setOpen={(value) => {
            if (!value) closeModal();
          }}
          addToWishlistHandler={modalPayload.addToWishlistHandler}
          removeFromWishlistHandler={modalPayload.removeFromWishlistHandler}
          isWishlisted={getIsWishlisted(modalPayload?.item?.id)}
        />
      )}
      {modalPayload && !isFoodItem(modalPayload.item) && (
        <ModuleModal
          open
          handleModalClose={closeModal}
          configData={configData}
          productDetailsData={modalPayload.item}
          addToWishlistHandler={modalPayload.addToWishlistHandler}
          removeFromWishlistHandler={modalPayload.removeFromWishlistHandler}
          isWishlisted={getIsWishlisted(modalPayload?.item?.id)}
        />
      )}
    </>
  );
};

export default MarketplaceFlashDealsSection;
