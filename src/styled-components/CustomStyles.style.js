...
const normalizeAddressTypeCompare = (v) => {
	if (v == null || v === "") return "";
	const s = String(v).trim().toLowerCase();
	return s === "others" ? "other" : s;
};

export const AddressTypeStack = styled(Stack)(
	({ theme, addressType, value }) => {
		const selected =
			normalizeAddressTypeCompare(addressType) ===
			normalizeAddressTypeCompare(value);
		return {
			alignItems: "center",
			justifyContent: "center",
			padding: "10px",
			...
			borderRadius: "5px",
			background: selected ? alpha(theme.palette.primary.main, 0.2) : undefined,
			cursor: "pointer",
		};
	}
);
...
export const UserProfileTabs = styled(Tabs)(({ theme, isActive, isMobileTab }) => ({
	minHeight: "unset",
	"& .MuiTabs-indicator": {
		display: "none",
	},
	...
	"& .MuiButtonBase-root": {
		...
		[theme.breakpoints.down("sm")]: {
			minHeight: isMobileTab ? "28px" : "32px",
		},
	},
}));

export const UserProfileTab = styled(Button)(
	({ theme, page, item, marginright, fontSize, borderRadius, isMobileTab }) => {
		const isActive = item?.name === page;
		return {
			background: isActive
			...
			marginInlineEnd: marginright ? marginright : "8px",
			paddingBlockEnd: "6px",
			...
			[theme.breakpoints.down("sm")]: {
				marginInlineEnd: isMobileTab ? "4px" : "6px",
				paddingInlineEnd: isMobileTab ? "8px" : "10px",
				paddingInlineStart: isMobileTab ? "8px" : "10px",
				paddingBlockEnd: "5px",
