...
const CheckOutPage = () => {
  ...
  const { page, store_id, id, incomplete_payment, selected_cart_ids } = router.query;
  const {
  ...
  const selectedCartIds = String(selected_cart_ids || "")
    .split(",")
    .map((id) => Number(id))
    .filter((id) => Number.isFinite(id));
  const filteredSelectedCartList =
    page === "cart" && selectedCartIds.length > 0
      ? cartList.filter((item) => selectedCartIds.includes(item?.cartItemId))
      : cartList;
  const selectedCartList =
    page === "cart" && selectedCartIds.length > 0 && filteredSelectedCartList.length === 0
      ? cartList
      : filteredSelectedCartList;
